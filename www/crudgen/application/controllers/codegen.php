...
class Codegen extends CI_Controller {
    ...
    function writefile($file,$content){
        $file = preg_replace('/application/', '', $file);
        echo "<b>writing /Users/web/Sites/edu.ucsd.lchc.dev/application/modules/admin$file</b><br>";
        if(preg_match('/form_validation/', $file)){
            echo $content;
        }
        /*
        $fp = fopen("/Users/web/Sites/edu.ucsd.lchc.dev/application/modules/admin$file", 'w');
        if($fp){
            fwrite($fp, $content);
            fclose($fp);
        }else{
            echo "<b>could not write $file</b><br>";
        }
        */
