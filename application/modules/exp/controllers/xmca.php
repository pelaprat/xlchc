...
class Xmca extends CI_Controller{
    ...
    function index(){
        ...
        $this->load->view(($simplehtml ? "view_simple_header" : "view_header"), $data);
        $this->load->view("view_xmca_top");
        ...
        if("" != $data['threadId']){
            $this->load->view("view_xmca_single_thread", $data);            
        }
        else if("" === $data['words']){
            ...
            $this->load->view("view_xmca_threads", $data);
        }
        else{                        
            $startDateInt = -1;
            $endDateInt = -1;
            ...
            if(isset($_GET['submit_timeline'])){
                ...
                $this->load->view("view_xmca_timeline", $data);
            }
            else{
                $data['xmca_search'] = $this->Xmca_sphinx_model->searchMessageBodies($data['words'],
                                                                                     ...
                                                                                     $startDateInt, 
                                                                                     $endDateInt);
                $this->load->view("view_xmca_search", $data);
            }
        }
        
        $this->load->view("view_xmca_bottom", $data);
        $this->load->view(($simplehtml ? "view_simple_footer" : "view_footer"), $data);
    ...
    function timeline(){
        ...
        $this->load->view(($simplehtml ? "view_simple_header" : "view_header"), $data);
        $this->load->view("view_xmca_top");
        $this->load->view("view_xmca_timeline");
        $this->load->view("view_xmca_bottom", $data);
        $this->load->view(($simplehtml ? "view_simple_footer" : "view_footer"), $data);        
